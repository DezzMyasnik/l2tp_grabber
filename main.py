...
#for pac in a:
#    if pac.proto == 115:
#        adress = process_sub_chan(pac.load[12:],pac.prio)

"""
for key in adresses:
    print(key)
    for sub_key in adresses[key]:
        print(f'\t{sub_key}')
        for sub_sub in adresses[key][sub_key]:
            print(f'\t\t{sub_sub}')
            print(len(adresses[key][sub_key][sub_sub]))
            for _sub in adresses[key][sub_key][sub_sub]:
